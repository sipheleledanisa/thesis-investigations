...
class MASQL_lsd(MARLAlgorithm):
    def __init__(
        ...
        super(MASQL_lsd, self).__init__(**base_kwargs)
    ...
    def _create_svgd_update(self):
        ...
        expanded_critic = tf.expand_dims(svgd_target_values,-1)
        ...
        hutch_estimator = noise*critic_grads*noise
        for iters in range(n_fixed_actions):
            noise =  tf.random_normal((1, self._value_n_particles, self._action_dim))
            hutch_estimator+= noise*critic_grads*noise
        hutch_estimator=hutch_estimator/n_fixed_actions

        LSD = ((expanded_critic)*grad_log_p+ hutch_estimator)


        action_gradients = LSD
