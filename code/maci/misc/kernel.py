...
def adaptive_isotropic_gaussian_kernel(xs, ys, h_min=1e-3):
    ...
    # Construct the second derivate with trace
    ...
    diff_expanded=tf.expand_dims(diff,-1) # ... x Kx x Ky x D x 1
    ...
    kappa_grad_grad_= tf.reduce_sum(kappa_grad_grad,-2) # ... Kx x Ky x D

    return {"output": kappa, "gradient": kappa_grad, "tr_kappa_grad_grad": kappa_grad_grad_}